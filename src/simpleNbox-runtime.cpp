...
fluxpool SimpleNbox::npp(std::string biome, double time) const {
  ...
  // Only LUE
   if (time == Core::undefinedIndex()) {
      npp = npp * co2fert.at(biome) * (1 + (0.001957 - 2 * 0.0002889 * temp_double) * (1 / (0.2735 + 0.001957 * temp_double - 0.0002889 * temp_double * temp_double)) * temp_double); // that's why used here instead of []
   } else {
      npp = npp * calc_co2fert(biome, time) * (1 + (0.001957 - 2 * 0.0002889 * temp_double) * (1 / (0.2735 + 0.001957 * temp_double - 0.0002889 * temp_double * temp_double)) * temp_double);
   }
