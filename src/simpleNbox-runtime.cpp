...
fluxpool SimpleNbox::npp(std::string biome, double time) const {
  ...
  // Only LUE
   if (time == Core::undefinedIndex()) {
      npp = npp * co2fert.at(biome) * (1 + (0.001524 - 2 * 0.0002877 * temp_double) * (1 / (0.2749 + 0.001524 * temp_double - 0.0002877 * temp_double * temp_double)) * temp_double); // that's why used here instead of []
   } else {
      npp = npp * calc_co2fert(biome, time) * (1 + (0.001524 - 2 * 0.0002877 * temp_double) * (1 / (0.2749 + 0.001524 * temp_double - 0.0002877 * temp_double * temp_double)) * temp_double);
   }
