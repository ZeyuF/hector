...
// CUETRENDYmin
   if (time == Core::undefinedIndex()) {
      npp = npp * co2fert.at(biome) * (1 + (-0.0025) * ( 1 / (-0.0025 * temp_double + 0.46) ) * temp_double ); // that's why used here instead of []
   } else {
      npp = npp * calc_co2fert(biome, time) * (1 + (-0.0025) * ( 1 / (-0.0025 * temp_double + 0.46) ) * temp_double );
   }
